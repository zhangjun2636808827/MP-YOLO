...
class MPBlockV2(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1, p=1, g=1, d=1, act=True):
        ...
        self.pad3 = nn.ZeroPad2d(padding=((p, p, 0, 2*p)))#(1,0,2,1)
        self.pad4 = nn.ZeroPad2d(padding=((0, 2*p,0 , 2*p)))#(0,1,1,2)
        self.bias_conv1 = Conv(c1, c2 // 2, k, s=s, p=0)
        self.bias_conv2 = Conv(c1, c2 // 2, k, s=s, p=0)
        self.bias_conv3 = Conv(c1, c2 // 2, k, s=s, p=0)
        self.bias_conv4 = Conv(c1, c2 // 2, k, s=s, p=0)
    ...
    def forward(self, x):
        ...
        y3 = self.bias_conv3(self.pad3(x))
        y4 = self.bias_conv4(self.pad4(x))
        return torch.cat([y1+y4, y2+y3], dim=1)

# class MPCBottleneck(nn.Module):#v1
...
#         return result
# class MPCBottleneck(nn.Module):#v2
...
#         result = x + self.conv(y5) if self.add else self.conv(y5)
#         return result
# class MPCBottleneck(nn.Module):#v3
...
#         y9 = torch.cat([y1, y2, y3, y4,y5,y6,y7,y8], dim=1)

#         result = x + self.conv(y9) if self.add else self.conv(y9)
#         return result
